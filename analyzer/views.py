...
def analyze_pdf(request):
    """Handle PDF file upload and analyze it (Ollama)."""
    ...
    try:
        ...
        # This happens on Render because Ollama is NOT running there
        return JsonResponse(
            {"error": "AI service is unavailable (Ollama not running)."},
            status=503
        )
    except ValueError as e:
        # e.g. "Ollama response was not valid JSON..."
    ...
    return JsonResponse({"success": True, "redirect": reverse("results")})


@require_http_methods(["POST"])
def analyze_text(request):
    """Handle raw contract text and analyze it (Ollama)."""
    try:
